...
struct ExtensionsFactory {
    api: Arc<Mutex<charger_service::mock::MockCharger>>,
}
...
impl sc_client_api::execution_extensions::ExtensionsFactory for ExtensionsFactory {
    fn extensions_for(
        ...
        if capabilities.has(sp_core::offchain::Capability::OffchainWorkerDbWrite) {
            ext.register(ChargerExt::new(
                charger_service::runtime::externalities::ChargerExternalities::new(
                    self.api.clone(),
                ),
            ));
        }
...
> {
    ...
    let api = Arc::new(Mutex::new(charger_service::mock::MockCharger::new()));
    client
        .execution_extensions()
        .set_extensions_factory(Box::new(ExtensionsFactory { api }));
...
pub fn new_full(mut config: Configuration) -> Result<TaskManager, ServiceError> {
    ...
    if config.dev_key_seed.is_some() {
        ...
        log::info!("Loading dev payment validator keystore");
        let key_type = sp_core::crypto::KeyTypeId(*b"paym");
        sp_keystore::SyncCryptoStore::insert_unknown(
            &*keystore,
            key_type,
            "//Eve",
        ...
        .expect("cannot insert payment validator key");
    }
