...
use sp_runtime::{
	...
	traits::{BlakeTwo256, IdentifyAccount, IdentityLookup, Verify, Hash},
};
...
fn should_register_new_consent() {
	new_test_ext().execute_with(|| {
		...
		let charger = Public::from_raw(hex!("bec4ab0eaff1a0d710274b3648bc5b2253e2bdee293987123962688f08a5c317"));
		let session_id = <Test as frame_system::Config>::Hashing::hash(&user);
		assert_ok!(UserConsent::new_consent_for_user(Origin::signed(user), charger, session_id));
	});
}
