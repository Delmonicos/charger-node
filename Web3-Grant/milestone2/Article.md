...
But moving from internal combustion engine vehicles to electric vehicles is not as easy as it seems. Most people are not convinced by electromobility due to the difficulty to locate a charging station, to connect into it, to the lack of transparency and finally due to the charging time.
...
At Delmonicos, we are convinced that blockchain technology can ease electromobility adoption by optimizing the charging process while enhancing security, scalability and upgrading the user experience.
...
* **CPO** (Charge Point Operator) manages and supervises the infrastructure of charging stations. It can operate one or many charging infrastructure on behalf of station owners or for its own account. CPO uses **CPMS** (Charge Point Managing System) to manage and supervise the charging stations.
* **eMSP** (eMobility Service Provider) is providing mobility services like giving access to charging stations to EV users through an RFID card of mobile App. 
* **Aggregator** (or Clearing House or hub) is a platform offering services that enable mobility operators to easily build partnerships between CPO and eMSP to enable electric mobility roaming.

Those three players are the main ones, but there are other secondaries like **OEM Charge Points Manufacturers** who build Charging Points, **NSP** (Navigation Service Provider) who provides location information of Charge Points, **Integrators** who support new players who want to setup their own Charge Station...

Moreover, some players might have several roles like a CPO who can be also eMSP while developping its own CPMS.

To help all these players to communicate together, some standards have been setting up to manage the electromobility ecosystem.

## A complex value chain handled by a multitude of protocols
Every distributed system has at least one protocol that defines the common language, also called standard. This language is used by each stakeholder to communicate and exchange data and values. 

In the electromobility world, there are mainly 2 protocols that are defined as the standards by the market (OCPP and OCPI). But there are many other protocols that are still used as shown in the image below.
...
### OCPP protocol
[OCPP][4] (Open Charge Point Protocol) allows the communication between **EVSE** and **CPMS**. 

This protocol exists in two major versions 1.6 and 2.0.1. The 1.6 is the most used version, the 2.0.1 should replace the 1.6 as it allows to handle more complexes CPMS and makes OCPP compatible with the ISO15118 features.
...
- in replacement of the private protocols used by the aggregators to exchange with CPO and eMSP.
...
The ISO 15-118 standard is composed of multiple sub-documents based on the [OSI model][7] (ISO 7498).

The ISO 15-118 proposes several development points of improvements : 
- a concrete implementation of Plug & Charge
- a smart charging solution allowing an intelligent management of the recharge. Using shared data connections between each tier of the charging process, smart charging allows to monitor, manage, and restricts the use of charging devices and optimizes energy consumption.  
- a bidirectional charging: V2H (vehicle to home), V2B (vehicle to building), V2G (vehicle to grid). Bidirectional charging allows to re-inject battery energy into the electrical grid to participate in the optimization of consumption peaks according to production peaks.
- several more advanced scenarios requiring sophisticated equipment such as charging using induction or automated charging.

[This standard is now considered unavoidable in the electromobility world][8]. However, it is not yet fully implemented, because it requires significant strategical decisions and financial risks for the players and needs a joint participation from a major part of the stakeholders who must cooperate together. Today, only a few implementation initiatives have been published, often resulting from an R&D dynamic rather than from industrialization in production.

Finally, ISO 15-118 proposes a security management via a PKI (Public Key Infrastructure) management architecture allowing the exchange of keys and certificates between stakeholders. This problematic requirement lies in the tree-like aspect of the certification authorities, which necessarily implies a hierarchical governance involving strategic thinking, each player wanting to be above the others, and [blocking a rapid acceptation of this kind of governance][9].
...
As seen above, the electrical ecosystem is complex, with many stakeholders and with many different visions. To handle this complexity, we develop a very modular solution allowing an easy adaptation and facilitate our market entry.
...
The purpose of Delmonicos is to provide an electric driver experience close to what a "Tesla" owner lives on a "Plug & Charge" Supercharger.
The user would only connect his vehicle to the charging station to start charging. No identification stress or pricing surprise. In the meantime,  starting from the MVP, Delmonicos will provide a "Pay as you go" solution, allowing the process of charging without subscription using a standard payment mode (CB, Paypal, Apple Pay, ...) without any additional fee, and of course, disruptive payment solution.

Our roadmap is etablished and in our future versions, Delmonicos would deliver more features based on this modular approach (Smart charging, offline charge, and more surprises).

![Benefits Delmonicos](./benefits_Delmonicos.png)

To achieve this challenge, Delmonicos is using Blockchain technology based on the principle of [trusted third parties][11] that will provides EV Drivers a new generation of simple and interconnected solution creating a universal network of charging station accessible, anywhere, anytime at the right price.

### Why [blockchain][19] for electromobility?  
The target is to propose a [new solution][20] being able to meet the requirements of the ecosystem being:
* identification of the ev-driver / vehicle / any element plugged on a charge point 
* authentication 
* secure payment
* governance

The mandatory features of the solution are the following:
...
Considering the above-mentioned criteria, we have made the choice to develop our solution around blockchain technology which is a perfect match for the ecomobility requirements :

* It is secure by design and nearly-impossible to hack if carefully used and implemented.
